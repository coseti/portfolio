      ...
      >
        <div className="mx-auto flex h-[72px] max-w-[1100px] items-center justify-between px-5 sm:px-8">
          ...
          <button
            ...
            aria-expanded={open}
            className="flex h-10 w-10 flex-col items-center justify-center gap-[5px] rounded-[10px] sm:hidden"
          >
            <span
          ...
          <nav
            ...
            } fixed left-0 right-0 top-[72px] flex flex-col items-start gap-0 border-b border-border bg-background px-5 pb-6 pt-3 transition-all duration-300 sm:static sm:flex-row sm:items-center sm:gap-6 sm:border-0 sm:bg-transparent sm:p-0`}
          >
            {SECTIONS.map((id) => (
              <a
                key={id}
                ...
                className="w-full border-b border-border py-3 text-[1.05rem] font-medium text-muted transition-colors hover:text-foreground sm:w-auto sm:border-0 sm:py-0 sm:text-[0.92rem]"
              >
                {t(id)}
              </a>
            ))}
            <div className="mt-4 flex w-full items-center gap-2 border-l-0 pl-0 pt-4 sm:ml-4 sm:mt-0 sm:w-auto sm:border-l sm:border-border sm:pl-4 sm:pt-0">
              <LocaleSwitcher />
              <a
